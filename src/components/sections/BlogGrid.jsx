...
var BlogGrid = function (_a) {
    var searchTerm = _a.searchTerm, selectedCategory = _a.selectedCategory;
    var _b = useState(blogArticles), filteredArticles = _b[0], setFilteredArticles = _b[1];
    var searchParams = useSearchParams();
    useEffect(function () {
        var filtered = blogArticles;
        // Filtrage par recherche
        var search = searchTerm || searchParams.get('search') || '';
        if (search) {
            filtered = filtered.filter(function (article) {
                return article.title.toLowerCase().includes(search.toLowerCase()) ||
                    article.excerpt.toLowerCase().includes(search.toLowerCase()) ||
                    article.tags.some(function (tag) { return tag.toLowerCase().includes(search.toLowerCase()); });
            });
        }
        ...
        if (selectedCategory && selectedCategory !== 'tous') {
            ...
            var categoryName_1 = categoryMap[selectedCategory];
            if (categoryName_1) {
                filtered = filtered.filter(function (article) { return article.category === categoryName_1; });
            }
        }
        setFilteredArticles(filtered);
    }, [searchTerm, selectedCategory, searchParams]);
    var featuredArticle = filteredArticles.find(function (article) { return article.featured; });
    var regularArticles = filteredArticles.filter(function (article) { return !article.featured; });
    return (<section className="py-20 bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        
        {/* Indicateur de filtrage */}
        {(searchTerm || (selectedCategory && selectedCategory !== 'tous')) && (<div className="mb-8 p-4 bg-blue-50 rounded-xl border border-blue-200">
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-4">
                <span className="text-blue-800 font-medium">
                  Filtres actifs :
                </span>
                {searchTerm && (<span className="bg-blue-100 text-blue-800 px-3 py-1 rounded-full text-sm">
                    Recherche : "{searchTerm}"
                  </span>)}
                {selectedCategory && selectedCategory !== 'tous' && (<span className="bg-blue-100 text-blue-800 px-3 py-1 rounded-full text-sm">
            ...
            <div className="bg-gradient-to-r from-primary-blue to-primary-blue/80 rounded-3xl overflow-hidden shadow-2xl">
              <div className="grid lg:grid-cols-2 gap-0">
                <div className="p-8 lg:p-12 text-white">
                  ...
                  <Link href={"/blog/".concat(featuredArticle.id)}>
        ...
        {/* Grille d'articles */}
        {regularArticles.length > 0 ? (<div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {regularArticles.map(function (article) { return (<article key={article.id} className="group bg-white rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-300 overflow-hidden hover:-translate-y-2">
                ...
                <div className="p-6">
                  ...
                  <div className="flex items-center justify-between">
                    <Link href={"/blog/".concat(article.id)} className="inline-flex items-center text-primary-blue font-semibold hover:text-primary-yellow transition-colors group">
                      Lire la suite
                  ...
                  {article.relatedFormations && article.relatedFormations.length > 0 && (<div className="mt-4 pt-4 border-t border-gray-200">
                      <p className="text-xs text-gray-500 mb-2">Formations liées :</p>
                      <div className="flex flex-wrap gap-2">
                        {article.relatedFormations.slice(0, 2).map(function (formation, index) { return (<Link key={index} href={formation.url}>
            ...
            <div className="max-w-md mx-auto">
              ...
              <OptimizedButton variant="secondary" size="md" onClick={function () { return window.location.reload(); }}>

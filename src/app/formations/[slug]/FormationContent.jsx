...
export default function FormationContent(_a) {
    var formation = _a.formation;
    var imageUrl = getImageURL(formation.imageData, formation.image);
    var hasImage = imageUrl && imageUrl !== '/images/formations/formations-hero.jpg';
    var rncpUrl = getRncpUrl(formation.rncp, formation.rncpUrl);
    return (<>
      {/* Hero */}
      <section className="pt-24 pb-8 relative min-h-[500px]">
        {hasImage ? (<div className="absolute inset-0">
            <img src={imageUrl} alt={formation.title} className="w-full h-full object-cover"/>
            <div className="absolute inset-0 bg-gradient-to-t from-white via-white/50 to-transparent"/>
          </div>) : (<div className="absolute inset-0 bg-gradient-to-br from-primary-blue to-indigo-800"/>)}
      </section>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            ...
            <div className="flex flex-wrap items-center gap-3 mb-4">
              ...
              {rncpUrl && (<a href={rncpUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center space-x-1 bg-primary-yellow/20 text-primary-blue px-3 py-1.5 rounded-full text-sm font-medium hover:bg-primary-yellow/40 transition-colors">
      ...
      <section className="py-8 bg-gray-50 border-b">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-6">
            {formation.duree && (<div className="flex flex-col items-center text-center">
            ...
            {formation.volumeHoraire && (<div className="flex flex-col items-center text-center">
            ...
            {formation.tauxReussite && (<div className="flex flex-col items-center text-center">
            ...
            {formation.tauxInsertion && (<div className="flex flex-col items-center text-center">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid lg:grid-cols-3 gap-12">
            <div className="lg:col-span-2 space-y-12">
              ...
              {/* Objectifs */}
              {formation.objectifs && (<motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
                  ...
                  <div className="bg-gradient-to-br from-primary-blue/5 to-indigo-50 rounded-2xl p-8">
                    {Array.isArray(formation.objectifs) ? (<ul className="space-y-3">
                        {formation.objectifs.map(function (obj, i) { return (<li key={i} className="flex items-start space-x-3">
                            <CheckCircle className="w-5 h-5 text-green-500 mt-0.5 flex-shrink-0"/>
                            <span className="text-gray-700">{obj}</span>
                          </li>); })}
                      </ul>) : (<div className="prose prose-gray max-w-none">
              ...
              {/* Programme */}
              {formation.programme && (<motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
                  <h2 className="text-2xl font-montserrat font-bold text-primary-blue mb-6 flex items-center">
                    <BookOpen className="w-6 h-6 mr-3 text-primary-yellow"/>Programme de formation
                  </h2>
                  <div className="space-y-4">
                    {Array.isArray(formation.programme) ? (formation.programme.map(function (module, i) {
                // Gérer différents formats de données
                var titre = module.titre || module.title || module.name || "Module ".concat(i + 1);
                var contenu = module.contenu || module.content || module.items || [];
                var isString = typeof module === 'string';
                return (<div key={i} className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm hover:shadow-md transition-shadow">
                            ...
                            {!isString && Array.isArray(contenu) && contenu.length > 0 && (<ul className="space-y-2 ml-11">
                                {contenu.map(function (item, j) { return (<li key={j} className="flex items-start space-x-2 text-gray-600">
              ...
              {/* Débouchés */}
              {formation.debouches && (<motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
                  <h2 className="text-2xl font-montserrat font-bold text-primary-blue mb-6 flex items-center">
                    <Briefcase className="w-6 h-6 mr-3 text-primary-yellow"/>Débouchés professionnels
                  </h2>
                  <div className="bg-gradient-to-r from-green-50 to-emerald-50 rounded-2xl p-8">
                    {Array.isArray(formation.debouches) ? (<div className="grid md:grid-cols-2 gap-3">
                        {formation.debouches.map(function (d, i) { return (<div key={i} className="flex items-center space-x-3 bg-white p-3 rounded-lg shadow-sm">
                            <CheckCircle className="w-5 h-5 text-green-500 flex-shrink-0"/>
                            <span className="text-gray-700 font-medium">{d}</span>
                          </div>); })}
                      </div>) : (<div className="prose prose-gray max-w-none">
                        <div className="text-gray-700 leading-relaxed whitespace-pre-line">
                          {formation.debouches}
                        </div>
                      </div>)}
              ...
              {/* Prérequis */}
              {formation.prerequis && (<motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
                  <h2 className="text-2xl font-montserrat font-bold text-primary-blue mb-6 flex items-center">
                    <FileText className="w-6 h-6 mr-3 text-primary-yellow"/>Prérequis et admission
                  </h2>
                  <div className="bg-orange-50 rounded-2xl p-8">
                    {Array.isArray(formation.prerequis) ? (<ul className="space-y-3">
                        {formation.prerequis.map(function (p, i) { return (<li key={i} className="flex items-start space-x-3">
              ...
              {/* Évaluation */}
              {formation.modalite && (<motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
                  <h2 className="text-2xl font-montserrat font-bold text-primary-blue mb-6 flex items-center">
                    <Award className="w-6 h-6 mr-3 text-primary-yellow"/>Modalités d'évaluation
                  </h2>
                  <div className="bg-purple-50 rounded-2xl p-8">
                    {Array.isArray(formation.modalite) ? (<ul className="space-y-3">
                        {formation.modalite.map(function (e, i) { return (<li key={i} className="flex items-start space-x-3">
              ...
              {/* Poursuites */}
              {formation.poursuiteEtudes && (<motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
                  <h2 className="text-2xl font-montserrat font-bold text-primary-blue mb-6 flex items-center">
                    <GraduationCap className="w-6 h-6 mr-3 text-primary-yellow"/>Poursuites d'études
                  </h2>
                  <div className="bg-blue-50 rounded-2xl p-8">
                    {Array.isArray(formation.poursuiteEtudes) ? (<ul className="space-y-3">
                        {formation.poursuiteEtudes.map(function (p, i) { return (<li key={i} className="flex items-start space-x-3">
            ...
            <div className="space-y-6">
              <motion.div initial={{ opacity: 0, x: 30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="bg-white border border-gray-200 rounded-2xl p-6 sticky top-8 shadow-lg">
                ...
                <div className="space-y-4">
                  {formation.repartition && (<div className="bg-gray-50 rounded-lg p-4">
                  ...
                  {(formation.cout || formation.financement) && (<div className="bg-green-50 rounded-lg p-4">
                  ...
                  {formation.certificateur && (<div className="bg-blue-50 rounded-lg p-4">
                  ...
                  {formation.contact && (<div className="bg-primary-yellow/10 rounded-lg p-4">
                      <div className="font-semibold text-gray-800 mb-2">Contact</div>
                      {formation.contact.telephone && (<div className="flex items-center space-x-2 text-sm text-gray-700 mb-1">
                          <Phone className="w-4 h-4 text-primary-blue"/><span>{formation.contact.telephone}</span>
                        </div>)}
                      {formation.contact.email && (<div className="flex items-center space-x-2 text-sm text-gray-700 mb-1">
                          <Mail className="w-4 h-4 text-primary-blue"/><span>{formation.contact.email}</span>
                        </div>)}
                      {formation.contact.adresse && (<div className="flex items-center space-x-2 text-sm text-gray-700">
                ...
                <div className="mt-6 space-y-3">
                  <button onClick={function () { return window.open('https://cma-education.ymag.cloud/index.php/preinscription/', '_blank'); }} className="w-full bg-primary-blue text-white py-3 rounded-lg font-semibold hover:bg-blue-700 transition-colors flex items-center justify-center space-x-2">
              ...
              {formation.entreprisesPartenaires && formation.entreprisesPartenaires.length > 0 && (<motion.div initial={{ opacity: 0, x: 30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="bg-gray-50 rounded-2xl p-6">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {formation.entreprisesPartenaires.map(function (e, i) { return (<span key={i} className="bg-white px-3 py-1 rounded-full text-sm text-gray-700 border">{e}</span>); })}
                  </div>
                </motion.div>)}
            </div>
          </div>
        ...
        <div className="max-w-4xl mx-auto px-4 text-center">
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <button onClick={function () { return window.open('https://cma-education.ymag.cloud/index.php/preinscription/', '_blank'); }} className="bg-white text-primary-blue px-8 py-3 rounded-full font-semibold hover:bg-gray-100 transition-colors flex items-center justify-center space-x-2">

...
import { 
  ...
  GraduationCap, Euro, MapPin, Clock, Zap
} from 'lucide-react'
...
// Interface complète pour PageEntreprise - 100% modifiable via Strapi
interface PageEntrepriseData {
  heroTitre: string
  heroSousTitre: string
  heroDescription: string
  heroBoutonPrincipal: string
  heroBoutonSecondaire: string
  sectionInvestirTitre: string
  sectionInvestirSousTitre: string
  sectionThematiquesTitre: string
  sectionThematiquesSousTitre: string
  sectionThematiquesDescription: string
  surMesureTitre: string
  surMesureDescription: string
  surMesureBouton: string
  modalitesTitre: string
  modalitesSousTitre: string
  modalitesDescription: string
  tarifTitre: string
  tarifJour: string
  tarifJourLabel: string
  tarifDescription: string
  tarifIntra: string
  financementTitre: string
...
// Valeurs par défaut
const defaultPageData: PageEntrepriseData = {
...
const defaultAvantages: EntrepriseService[] = [
...
const defaultThematiques: FormationThematique[] = [
...
const defaultModalites: EntrepriseModalite[] = [
...
export default function FormationsEntreprises() {
  const [avantages, setAvantages] = useState<EntrepriseService[]>(defaultAvantages)
  ...
  useEffect(() => {
    async function loadData() {
      try {
        const [servicesData, thematiquesData, modalitesData, pageData] = await Promise.all([
          getEntrepriseServices(),
          getFormationThematiques(),
          getEntrepriseModalites(),
          getPageEntreprise()
        ])
        if (servicesData?.length) setAvantages(servicesData as EntrepriseService[])
        if (thematiquesData?.length) setThematiques(thematiquesData as FormationThematique[])
        if (modalitesData?.length) setModalites(modalitesData as EntrepriseModalite[])
        if (pageData) setPage({ ...defaultPageData, ...pageData } as PageEntrepriseData)
      } catch (error) {
              ...
              <div className="relative">
                <div className="bg-white/10 backdrop-blur-xl rounded-3xl p-8 border border-white/20">
                  ...
                  <div className="space-y-4">
                    {["Lean Construction", "Rénovation énergétique", "BIM collaboratif"].map((item, i) => (
                ...
                <motion.div
                  ...
                  className="absolute -top-4 -right-4 bg-primary-blue text-white px-4 py-2 rounded-full text-sm font-semibold shadow-lg"
                >
                  ✓ Financement OPCO
                </motion.div>
              </div>
          ...
          <div className="grid md:grid-cols-2 gap-8">
            {avantages.map((avantage, index) => (
              <motion.div
                ...
                className="group p-8 bg-gradient-to-br from-gray-50 to-white rounded-2xl border border-gray-100 hover:border-orange-200 hover:shadow-xl transition-all"
              >
              ...
              <div className="space-y-4">
                {thematiques.map((thematique, index) => (
                  <motion.div
                    ...
                    viewport={{ once: true }}
                    transition={{ delay: index * 0.1 }}
                    className="flex items-start gap-4 p-4 bg-white rounded-xl border border-gray-100 hover:border-orange-200 hover:shadow-md transition-all group"
                  >
                    ...
                    <div>
                      ...
                      {thematique.description && (
                        <p className="text-sm text-gray-500 mt-1">{thematique.description}</p>
                      )}
                    </div>
                  </motion.div>
                ))}
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {modalites.map((modalite, index) => (
              <motion.div
                ...
                className="group text-center p-8 bg-gray-50 rounded-2xl border-2 border-transparent hover:border-orange-500 hover:bg-white hover:shadow-xl transition-all"
              >
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid lg:grid-cols-2 gap-12">
            ...
            >
              ...
              <div className="space-y-4">
                {page.financements.map((financement, index) => (
                  <motion.div
                    key={index}
                    initial={{ opacity: 0, x: 20 }}
                    whileInView={{ opacity: 1, x: 0 }}
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          >
            ...
            <div className="flex flex-col sm:flex-row gap-6 justify-center items-center">
              <a 
                href={`tel:${page.telephone.replace(/\s/g, '')}`}

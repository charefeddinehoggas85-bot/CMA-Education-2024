...
async function addFormationTPFinal() {
  try {
    console.log('🚀 Ajout de la formation Conducteur de Travaux Publics - Reconversion...');

    // 1. Utiliser la catégorie reconversion existante (ID: 2)
    const reconversionCategoryId = 2;
    console.log('✅ Utilisation de la catégorie Reconversion existante (ID: 2)');
    ...
    // 3. Données de la nouvelle formation
    const formationData = {
      data: {
...
<h3>À l'issue de la formation, vous serez capable de :</h3>
<ul>
<li>Planifier, organiser et suivre l'exécution de chantiers de travaux publics</li>
<li>Gérer les budgets, contrats, achats et marges financières d'un projet</li>
<li>Superviser les équipes sur le terrain et coordonner les différents corps de métier</li>
<li>Veiller au respect des délais, des normes de sécurité, de qualité et des enjeux environnementaux</li>
<li>Assurer la communication avec les riverains, les prestataires, les autorités locales et les donneurs d'ordre</li>
</ul>`,
        ...
        category: reconversionCategoryId,
        ordre: 3,
        isActive: true,
    ...
    if (response.status === 200 || response.status === 201) {
      ...
      // 5. Vérifier que la formation apparaît dans la catégorie reconversion
      console.log('\n🔍 Vérification de l\'assignation à la catégorie...');
      const formationsReconversionResponse = await axios.get(
        `${STRAPI_URL}/api/formations?populate=category&filters[category][id][$eq]=${reconversionCategoryId}`
      );
      ...
      const dropdownResponse = await axios.get(
        `${STRAPI_URL}/api/formation-categories/${reconversionCategoryId}?populate=formations`
      );
      
      const reconversionCat = dropdownResponse.data.data;
      ...
      console.log('\n✅ SUCCÈS: La formation est maintenant disponible dans le dropdown reconversion!');

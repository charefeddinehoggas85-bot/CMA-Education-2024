...
// Template de page de formation corrigé
const getPageTemplate = (folderName) => `import { Metadata } from 'next'
...
console.log('🔧 Correction des pages de formation...\n');

folders.forEach(folder => {
  const filePath = path.join('src/app/formations', folder, 'page.tsx');
  
  if (fs.existsSync(filePath)) {
    // Utiliser le nom du dossier comme slug
    const slug = folder;
    const content = getPageTemplate(slug);
    fs.writeFileSync(filePath, content);
    console.log(`✅ ${folder}/page.tsx corrigé`);
  } else {
    console.log(`⚠️  ${folder}/page.tsx non trouvé`);
  }
});
